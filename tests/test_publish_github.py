...
def test_create_github_pull_request_uses_default_branch():
    ...
    assert "head=owner%3Amagicmd%2Farticle" in str(requests[1].url)
    assert requests[2].method == "POST"
    assert requests[2].url.path == "/repos/owner/repo/pulls"
    assert '"head":"magicmd/article"' in requests[2].content.decode()
    assert '"base":"main"' in requests[2].content.decode()


def test_create_github_pull_request_returns_existing_open_pr():
    requests = []
