...
def test_site_validation_manifest_records_current_platform_baseline():
    ...
    assert manifest["generated_from"] == "output/csdn-complex-006-csdn-fixes/batch-report.md"
    ...
    assert csdn_items
    assert len(csdn_items) >= 10
    assert all(item["fetch_mode"] == "camoufox" for item in csdn_items)
    assert all(item["status"] == "converted" for item in csdn_items)
